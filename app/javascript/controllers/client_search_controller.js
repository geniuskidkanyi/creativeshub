...
export default class extends Controller {
  ...
  renderResults(clients, query) {
    ...
    // Always offer to create when something's been typed — search is fuzzy, so
    // loosely-related matches shouldn't hide the "create new" path.
    if (trimmed.length > 0) {
      const create = document.createElement("button")
      create.type = "button"
      create.setAttribute("data-action", "client-search#openModal")
      create.className = `w-full text-left px-3 py-2 text-sm text-primary hover:bg-muted/50 transition-colors flex items-center gap-2 ${clients.length ? "border-t border-border" : ""}`
      create.innerHTML = `
        ...
        <span>Create &ldquo;${this.escapeHtml(trimmed)}&rdquo;</span>
      `
      this.searchResultsTarget.appendChild(create)
